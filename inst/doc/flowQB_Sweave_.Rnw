...
\title{flowQB Package for Instrument Sensitivity Assessment: \\
An Automatic Q and B Calculation}
...
OV <- c(Q=as.double(QQB[1]), B=as.double(QQB[2]), Rsquared=as.double(round(QQB[3], 2)))
OV[1]
OV[2]
OV[3]
@
...
OV <- c(Q=as.double(LQB[1]), B=as.double(LQB[2]), Rsquared=as.double(round(LQB[3], 2)))
OV[1]
OV[2]
OV[3]
...
<<label=tab1,echo=TRUE,results=tex>>=
Coefs <- DiscriminantExamination(as.double(QQB[1]),as.double(QQB[2]),as.double(QQB[4]))
...
\bibitem{f}
F. El Khettabi et al. 2012, 
{\em Flow Cytometer Sensitivity: A Quadratic Model}, to be submitted. 
