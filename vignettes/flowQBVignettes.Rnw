...
\author{
    Josef Spidlen, Faysal El Khettabi, Wayne Moore, \\
...
bead set, based on K-means clustering and quadratic regression methods.
...
version \cite{wj,CH,HW}.

The code is already workingn and useable, but we don't have a proper
