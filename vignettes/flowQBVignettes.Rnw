...
\item \Rcode{max\_iterations} -- The maximum number of iterations for iterative
fitting with weight adjustments. In most cases, the fitting converges 
relativelly fast. The iterating stops when either the maximum of iterations is
