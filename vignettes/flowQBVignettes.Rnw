...
The \Rfunction{fit\_multipeak} function performs quadratic fitting for
...
Internally, this is the same \Rfunction{fit\_multipeak} function except
...
in case of the \Rfunction{fit\_multipeak} function.
This clustering is performed on data transformed with the Logicle
transformation \cite{Parks2006, Moore2012}.
Generally, the Logicle \textit{width} ($w$ parameter) of 0.5 has been working
well, but users can change the default by providing a different 
...
This argument is applicable to the \Rfunction{fit\_multipeak} function only;
the \Rfunction{fit\_spherotech} and \Rfunction{fit\_thermo\_fischer} functions 
have the number of peaks predefind to 8 and 6, resp.
...
spherotech_results <- fit_spherotech(fcs_file_path, scatter_channels, 
    ...
    max_iterations = max_iterations, logicle_width = 0.5)
## This is the same as if we were running
## fit_multipeak(fcs_file_path, scatter_channels, 
...
bead data by the 
\Rfunction{fit\_multipeak} (and \Rfunction{fit\_spherotech},
