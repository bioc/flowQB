...
if(nClusters >= 1) 
{
  ...
  for(jc in 1:nClusters)
  {
    ...
    percentiles <- as.numeric(quantile(vect, probs))
...
gPS <- function(TSSINGLETS,OSSINGLETS,nClusters)
{   
  # This function uses Gaussian distribution fitting R software "MASS" package  to extract the statistics for the peak found by Kmeans. 
  # TSSINGLETS table having the LOGICLE events VALUES of the selected marker including SSC in the first Column. 
  # OSSINGLETS table having the ORIGINAL events VALUES of the selected marker including SSC in the first Column. 
  # nClusters number of the bead populations. 

library("MASS")
MEANS=rep(0,nClusters)
SDs=rep(0,nClusters)
Nes=rep(0,nClusters)
x=data.frame(TSSINGLETS)
cl=kmeans(x,nClusters,500,500)
for(jc in 1:nClusters) {
eventsjc=which(cl$cluster==jc)
vect=OSSINGLETS[eventsjc,2]
fitgmass <- unlist(fitdistr(vect, 'normal'))
MEANS[jc]=fitgmass[1]
SDs[jc]=fitgmass[2]
Nes[jc]=length(eventsjc)
}

xnc=nClusters-1
Rdx=rep(0,xnc)

OrderedMeans1 <- order(MEANS)
MEANS1=MEANS[OrderedMeans1]
SD1=SDs[OrderedMeans1]
for(jc in 1:xnc)
{
  Rdx[jc]=(MEANS1[jc+1]-MEANS1[jc])/(1+SD1[jc+1]+SD1[jc])
}
FromLowToHighPeak=order(MEANS)
DATAX=data.frame(Nes[FromLowToHighPeak],MEANS[FromLowToHighPeak],SDs[FromLowToHighPeak],c(0,Rdx))
return(DATAX)

}


rPS <- function(TSSINGLETS,OSSINGLETS,nClusters)
{   
  # This function uses Robust Statistics  to extract the statistics for the peak found by Kmeans. 
  # TSSINGLETS table having the LOGICLE events VALUES of the selected marker including SSC in the first Column. 
  # OSSINGLETS table having the ORIGINAL events VALUES of the selected marker including SSC in the first Column. 
  # nClusters number of the bead populations. 
...
eventsjc=which(cl$cluster==jc)
vect=OSSINGLETS[eventsjc,2]
MEANS[jc]=median(vect)
SDs[jc]= median(abs(vect-median(vect)))*1.4826
Nes[jc]=length(eventsjc)
}
...
MultilevelgPS <- function(TSSINGLETS,OSSINGLETS,nClusters)
{   
  # This function uses Gaussian distribution fitting R software "MASS" package  to extract the statistics for the peak found by multi-Kmeans. 
  # TSSINGLETS table having the LOGICLE events VALUES of the selected markers. 
  # OSSINGLETS table having the ORIGINAL events VALUES of the selected markers. 
  # nClusters number of the bead populations. 

library("MASS")

MEANS=matrix(0,nClusters,dim(TSSINGLETS)[2])
SDs=matrix(0,nClusters,dim(TSSINGLETS)[2])
Nes=rep(0,nClusters)

x=data.frame(TSSINGLETS)
cl=kmeans(x,nClusters,600,600)
for(jc in 1:nClusters)
{
eventsjc=which(cl$cluster==jc)
vect=OSSINGLETS[eventsjc,]
jj=0
for( jjx in 1:dim(TSSINGLETS)[2] )
{
jj=jj+1
fitgmass <- unlist(fitdistr(vect[,jj], 'normal'))
MEANS[jc,jj]=fitgmass[1]
SDs[jc,jj]=fitgmass[2]
Nes[jc]=length(eventsjc)
}
}


OrderedMeans1 <- order(MEANS[,1])
DATAALL=data.frame(Nes,MEANS,SDs)
names(DATAALL)=c("NE",paste("gMean",1:dim(TSSINGLETS)[2],sep=""),paste("gSD",1:dim(TSSINGLETS)[2],sep=""))

return(DATAALL[OrderedMeans1,])


}


MultilevelrPS <- function(TSSINGLETS,OSSINGLETS,nClusters)
{   
  # This function uses Robust Statistics  to extract the statistics for the peak found by multi-Kmeans. 
  # TSSINGLETS table having the LOGICLE events VALUES of the selected markers. 
  # OSSINGLETS table having the ORIGINAL events VALUES of the selected markers. 
  # nClusters number of the bead populations. 
...
{
jj=jj+1
MEANS[jc,jj]=median(vect[,jj])
SDs[jc,jj]= median(abs(vect[,jj]-median(vect[,jj])))*1.4826
Nes[jc]=length(eventsjc)
...
OrderedMeans1 <- order(MEANS[,1])
DATAALL=data.frame(Nes,MEANS,SDs)
names(DATAALL)=c("NE",paste("rMean",1:dim(TSSINGLETS)[2],sep=""),paste("rSD",1:dim(TSSINGLETS)[2],sep=""))
...
qrWEIGHTEDMESF <- function(MESFmeanssd2, Peak1, Peak2,Givenweights)
{  
  ...
  #  Givenweights: a vector of weights to be used in the fitting process, 
     # See also lm function for more ‘Details’,
  ...
  QB <- lm(MESFvariances ~ MESFmeans + I(MESFmeans^2),weights = Givenweights)
  ...
  Pvalues=summary(QB)$coef[,4]
  StdError=summary(QB)$coef[,2]
    return(c(Q=Q, B=B, Rsquared=round(summary(QB)$r.squared, 2), sigmaS2=QB$coeff[3],Pb=Pvalues[1],Pq=Pvalues[2],Pcv=Pvalues[3],StdErrorb=StdError[1],StdErrorq=StdError[2],StdErrorcv=StdError[3]))
  }
  if(QB$coeff[2] == 0) 
  {
